...
* ^caseSensitive = true
* ^experimental = false
* #COMP "Composite View" "COMP distributions inform a subscribing source system that their records are out-of-sync with the Composite View. For non-source system, the Composite View distribution informs a subscriber when a data attribute has been modified on a specific source."
...
* #PHNJOIN "PHN Join" "A PHNJOIN distribution only works for subscribers that are source systems and informs source systems that their record has joined an entity with a PHN, or a PHN has joined its entity."
...
* #SSRISYNC "SSRI Sync" "SSRISYNC distributions apply only to subscribers that are source systems and are used to notify a parent source system when one of its child system’s records has joined or changed within an entity set. SSRISYNC distributions support PHSA and the Clinical and Systems Transformation project (CST). In this project, multiple sources (VCHA, PHC, and PHSA) are implementing a single Cerner CIS (VPP Cerner) accross all their facilities and replacing multiple existing systems. As part of this effort, all patients are issued a new SRI; existing SRIs are recorded and stored for historical use only. The new Cerner CIS is loading the SRI from the old system as the SSRI in the new system. The new CIS will be deployed to facilities over several years and the patients will be migrated from their existing systems to the new CIS as their facility goes live and the legacy system is decommissioned."
