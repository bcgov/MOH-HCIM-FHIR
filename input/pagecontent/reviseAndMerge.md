...
The Revise Patient interaction is used to add or update client information in the Client Registry.  If the demographics are updated on the point of service application, a ‘Revise Patient’ message must be automatically generated to the Client Registry. 
...
The record the point of service is keeping must be in the Client Registry prior to the merge or the Merge Person message will fail. This record is known as the ‘survivor’ record.
...
The asynchronous pattern for Revise and Merge Patient follow the same pattern as V3.  The figure 1 show the client requesting an operation from the Client Registry and the Client 2 responding, later, with the response.
