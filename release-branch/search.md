# Search Patient Operations - BC Client Registry FHIR Implementation Guide v1.0.0

* [**Table of Contents**](toc.md)
* **Search Patient Operations**
...
More details can be found in the [request Bundle](StructureDefinition-bc-get-demographics-request-bundle.md) and [response Bundle](StructureDefinition-bc-search-response-bundle.md) definitions.
...
The IN and OUT search parameters and resources for Get Demographics can be found here [GetDemographics search](OperationDefinition-bc-patient-get-demographics.md).

#### Find Candidates Parameters

The IN and OUT search parameters for Find Candidates can be found here [FindCandidates search](OperationDefinition-bc-patient-find-candidates.md).
...
(BC CR) Used for "score" of matching from EMPI. The observation event value is the matching generated by Initiate. This is a confidence scoring system that Initiate uses to link members. If the score is between 90 and 156 it indicates that the person returned is likely the same person based on teh attributes provided. A score greater than 156 indicates that the member returned has an extremely high probability of being the same person.
